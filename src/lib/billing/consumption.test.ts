...
function refuel(odometerKm: number, liters: number, tankLevelAfter: TankLevel | null): Refuel {
  return {
    id: `r-${odometerKm}`,
    liters,
...
describe('fullTankSamples', () => {
  it('calcola il consumo solo tra due pieni consecutivi', () => {
    const samples = fullTankSamples([
      refuel(10000, 40, 'full'),
      refuel(10500, 10, 'half'), // rifornimento parziale: non chiude un intervallo
      refuel(10600, 50, 'full'),
    ]);
    expect(samples).toEqual([600 / 50]);
  });

  it('ignora i pieni senza km percorsi', () => {
    expect(fullTankSamples([refuel(10000, 40, 'full'), refuel(10000, 5, 'full')])).toEqual([]);
  });

  it('non produce campioni con meno di due pieni', () => {
    expect(fullTankSamples([refuel(10000, 40, 'full')])).toEqual([]);
  });
});
...
describe('resolveConsumption', () => {
  it('usa il dato di libretto finché i campioni non bastano', () => {
    const result = resolveConsumption([refuel(10000, 40, 'full'), refuel(10500, 40, 'full')], 15);
  ...
  it('passa al consumo misurato con abbastanza pieni', () => {
    const refuels = [
      refuel(10000, 40, 'full'),
      refuel(10400, 40, 'full'), // 10 km/l
      refuel(10800, 40, 'full'), // 10 km/l
      refuel(11200, 40, 'full'), // 10 km/l
    ];
  ...
  it('scarta gli outlier oltre il 30% dalla mediana', () => {
    const refuels = [
      ...
      refuel(10400, 40, 'full'), // 10 km/l
      refuel(10800, 40, 'full'), // 10 km/l
      refuel(11600, 40, 'full'), // 20 km/l: outlier (rifornimento non davvero pieno)
      refuel(12000, 40, 'full'), // 10 km/l
    ];
    const result = resolveConsumption(refuels, 15);
    expect(result.sampleCount).toBe(3);
    expect(result.kmPerLiter).toBeCloseTo(10, 6);
